...
from database.users import get_user_by_id, update_user_balance

logger = logging.getLogger(__name__)

def generate_short_code(length=7) -> str:
    """Génère un identifiant court (ex: S7K9M2P)."""
    chars = string.ascii_uppercase + string.digits
    return ''.join(random.choices(chars, k=length))

def get_estimated_end_time(matches: list) -> datetime:
    """Calcule l'heure de fin estimée en se basant sur le coup d'envoi le plus tardif."""
...
# --- GESTION DU LEADERBOARD DYNAMIQUE (WINRATE, NETWORK, VOLUME) ---

def get_timeframe_details(period: str):
    """Retourne la date de début et le volume minimal requis selon la période."""
    now = datetime.now(timezone.utc)
    if period == "day":
...
def calculate_leaderboards(category: str, period: str):
    """Génère le classement complet selon la catégorie et la période spécifiées."""
    cutoff, min_volume = get_timeframe_details(period)
    
    # Récupérer les tickets et sessions de la période pour calculer le volume et winrate
    ...
    # Récupérer les nouveaux utilisateurs pour calculer le recrutement (Network)
    ...
    for uid, stats in user_stats.items():
        if uid == 0: continue # Ignorer Compte Don
        u = user_map.get(uid)
        if not u: continue
...
def safe_add_item(telegram_id: int, item_id: int):
    """Ajoute un Item au joueur en respectant la limite de 3 par sac."""
    u = get_user_by_id(telegram_id)
    if not u: return
    key = f"item_{item_id}_count"
    new_qty = min(u.get(key, 0) + 1, 3) # Cap strict à 3
...
def distribute_top_rewards(category: str, period: str, total_prize_pool: int, bonus_item: int = None):
    """Distribue les récompenses aux 10 premiers qualifiés et génère un reçu."""
    ...
    # Génération du code de paiement unique
