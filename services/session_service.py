...
def join_session(session_id: str, joiner_id: int, predictions: list):
    ...
    supabase.table("users").update({
        "coins_balance": int(joiner["coins_balance"]) - gross_fee
    }).eq("telegram_id", joiner_id).execute()
    ...
    if len(tickets) + 1 == session.get("max_participants", 2):
        ...
        if session["type"] == "DUEL":
            update_data["opponent_id"] = joiner_id
...
def resolve_session(session_id: str):
    ...
    supabase.table("sessions").update({
        "status": "COMPLETED",
        "winner_id": outcomes.get("winner_id")
...
def calculate_leaderboards(category: str, period: str, limit: int = 10):
    """
    Calcule le classement demandé par top.py.

    Périodes:
      - day   = 24 heures, seuil 500 Coins
      - week  = 7 jours, seuil 2000 Coins
      - month = 30 jours, seuil 5000 Coins

    Les joueurs sous le seuil restent dans le tableau et sont marqués
    ``qualified=False`` afin que l'interface puisse les afficher avec
    "❌ Volume Insuffisant".
    """
    period_config = {
        "day": (1, 500),
    ...
    if category not in {"winrate", "network", "volume"}:
        category = "volume"

    days, min_volume = period_config.get(period, period_config["week"])
    ...
    for session in sessions:
        tickets = get_tickets_for_session(session["id"])
        gross_fee = int(session.get("gross_entry_fee") or 0)
    ...
    if category == "winrate":
        board.sort(
            key=lambda p: (p["winrate"], p["volume"], p["network"]),
            reverse=True
        )
    elif category == "network":
        board.sort(
            key=lambda p: (p["network"], p["volume"], p["winrate"]),
            reverse=True
        )
    else:
        board.sort(
            key=lambda p: (p["volume"], p["winrate"], p["network"]),
            reverse=True
        )

    return board[:limit], min_volume

def get_dynamic_leaderboard(category: str, period: str, limit: int = 10):
    """
    Ancienne API de classement conservée pour compatibilité.

    Elle accepte les anciennes périodes 24h/7d/30d et retourne uniquement
    les joueurs qualifiés sous l'ancien format {telegram_id, username, score}.
    """
    period_aliases = {
        "24h": "day",
        "7d": "week",
    ...
    board, _ = calculate_leaderboards(category, normalized_period, limit=limit)
    ...
    if quota_str:
        try:
            supabase.table("app_settings").upsert({
                "setting_key": "api_quota",
                "setting_value": str(quota_str)
            }).execute()
        except Exception:
            pass
